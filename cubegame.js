...
app.controller("cubegame", function ($scope) {
    ...
    $scope.highscores = [
	  { name: 'TacoTaco', seed: 2, dif:"Standard", score: 2100 },
	  { name: 'Dj-Kippie', seed: 2, dif:"Standard", score: 600 },
	  { name: 'Mikers', seed: 3,dif:"Proffesional", score: 2250 },
      { name: 'Rick & Morty', seed: 6, dif:"Extreme", score: 700 }
    ];

    $scope.playerName = "New Player";
    $scope.difficultyChoice = "1";
    var difficulty1 = $scope.difficultyChoice;
    ...
    $scope.submit = function () {
        ...
        difficulty1 = $scope.difficultyChoice;
        seed = $scope.Aseed;
        randomSeed();
    ...
    document.body.onkeydown = function (e) {
        ...
        if (e.keyCode == 90) {
           // console.log("Z");
            if (startscreen == false && player.dead == true) {
                if (player.dead == true) {
                    highscoreUpdate();
    ...
    function highscoreUpdate() {
        ...
        var scoredif = "Standard";  

        if (difficulty1 == 1){
            scoredif = "Standard";
        };
        if (difficulty1 == 2){
            scoredif = "Proffesional";
        }
        if (difficulty1 == 10){
            scoredif = "Extreme";
        }
        ...
        if ($scope.highscores.length < 9 || score >= $scope.highscores[0].score) {
            $scope.highscores.push({ name: $scope.playerName, seed: seed, dif:scoredif, score: score })
        };
    };
    ...
    var boxseed = 0;
    function BoxUpdate() {
        if ((globaltimer / ((100 / difficulty1) + (outputSeed[boxseed] * 20))) % 1 === 0 && player.dead == false) { // USES SEED TO CREATE OBJECTS
    ...
    function PlayerUpdate() { 
        ...
        ctx.fillStyle = "#ca99ff";
        ctx.fillRect(-(player.width / 2), -(player.height / 2), player.width, player.height);

        ctx.lineWidth = "3";
        ctx.strokeStyle = "#25004d";
        ctx.beginPath();
        ...
        ctx.beginPath();
        ctx.rect(-player.width / 2, -(player.height / 2), 10, 10);
        ctx.stroke();

        ctx.fillRect((player.width / 2) - 10, (player.height / 2) - 10, 10, 10);
        ctx.beginPath();
        ctx.rect((player.width / 2) - 10, (player.height / 2) - 10, 10, 10);
        ctx.stroke();
    ...
    function startScreen() {
        ...
        for (var i = 0; i < 20; i++) {
            ctx.save();
            ctx.translate(Math.random() * c.height * 1.5, Math.random() * c.width / 2 );
            ctx.globalAlpha = (Math.sin(Math.random()*10) + 1) / 10 + 0.05;
            ctx.fillRect(0, 0, rboxsize = Math.random() * 500, rboxsize);
            ctx.restore();
        };
